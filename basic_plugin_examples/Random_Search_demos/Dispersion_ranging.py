# Provide an example of use of the AcousticChannel and CNNDetection plugins, as a method for
# acoustic ranging and dispersion. This example showcases a behavior of fencing,
# paired with random search and a primitive obstacle avoidance

from SwarmSwIM import Simulator, Plotter, CNNDetection, AcousticChannel
...
AVOIDANCE_RANGE = 3.5 #meters
BEACON_UPDATE_PERIOD = 1.5 #sec
RANDOM_UPDATE_PERIOD = 4.0 #sec

# Create a visual reference for the plot: A Circle of radius 20 meters
circle = plt.Circle((0, 0), RANGE_LIMIT, color='g', fill=False, alpha=0.5)
...
class AgentControl:
    def __init__(self, agent):
        ...
        self._next_random_time = S.time + RANDOM_UPDATE_PERIOD + S.rnd.uniform(high=4.0)
    ...
    def trigger_random (self, callback):
        if self._next_random_time<=S.time and self.isinrange:
            self._next_random_time += RANDOM_UPDATE_PERIOD
            callback()
    def random_correction(self):
        new_heading = (self.agent.cmd_heading + S.rnd.uniform(low=-30,high=30))%360
        # if not actively avoiding then apply random correction
    ...
    def range_cmd(self, output):
        ...
        # if ouside the range and not actively avoiding, try to return 
        if not self.isinrange and not self.active_avoidance:
    ...
    def avoidance_cmd(self):
        for key in self.agent.NNDetector:
            if key=='time_lapsed': continue
            # ignore detection above 3 meters
            if self.agent.NNDetector[key][0] > AVOIDANCE_RANGE: continue
            else:
                self.agent.cmd_heading = (self.agent.measured_heading - 20) % 360
                print(f'agent  {self.agent.name} avoids {self.agent.cmd_heading}')
                self.active_avoidance = True
                return
        # if no valid detection deactivate active avoidance
        self.active_avoidance = False
...
def animation_callback():
    ...
    new_detections = Detection(S) # TODO continue from here
    ...
    if new_detections:
        for name in new_detections:
                for ctrl in controllers:
                    if name==ctrl.agent.name: 
                        ctrl.avoidance_cmd()
